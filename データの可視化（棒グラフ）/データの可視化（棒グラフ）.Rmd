...
data_2017 <- data %>% filter(year == 2017) # 2017年のデータに絞る
```
...
data_2017 %>% 
  ...
  labs(x = "", y = "得票率", title = "2017年衆院選: 各政党の候補者の平均得票率")
```
...
正直、積み上げ棒グラフより線グラフの方が視認性が高いよね。  
使用は非推奨です。

```{r}
data %>% 
  ggplot(aes(x = fct_reorder(as.factor(year), voteshare, .desc = F),
             y = voteshare)) +
  geom_bar(aes(fill = party_jpn), stat = "identity") +
  labs(x = "実施年", y = "得票率", title = "1996～2017年衆院選: 各政党の候補者の平均得票率") +
  geom_label(aes(label = paste(round(voteshare, 1), "%", sep = ""), group = party_jpn),
             size = 3,
             position = position_stack(vjust = 0.5)) +
